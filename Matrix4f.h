...
public:
    ...
    Matrix4f static returnProjectionMatrix( float fov_y, float aspect, float zNear, float zFar);
...
Matrix4f Matrix4f::returnProjectionMatrix (float fov_y, float aspect, float zNear, float zFar) {
    auto ctan = [] (float x) -> float {
        return cos(x) / sin(x);
    };

    float f = ctan(Util::toRadians(fov_y) / 2.0f);
...
void Matrix4f::fillFloatArray(float *array){
    for( int i = 0 ; i < 4 ; i++ )
        for( int j = 0 ; j < 4 ; j++ )
            array[ i * 4 + j ] = mat[i][j];
}
