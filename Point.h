...
public:
    ...
    typedef typename std::remove_reference<decltype(container::array[0])>::type ellementType;
    ...
    friend std::ostream& operator<< ( std::ostream& stream, const Point& arg ){
        for( int i = 0; i < N; i++ )
            stream << arg.array[i] << ' ';
    ...
    friend std::istream& operator>> ( std::istream& stream, Point& arg ){
        for( int i = 0; i < N; i++ )
            stream >> arg.array[i];
